...
$backup = "$rankingFile.backup-classement-premium-podium-$stamp"
...
# 1) Le classement garde toute sa logique actuelle. On remplace uniquement
# l'affichage du podium et l'habillage de la liste #4+.
if ($content -notmatch '<PodiumBoard positions=\{podiumData\} viewerTheme=\{viewerTheme\} />') {
    Copy-Item $backup $rankingFile -Force
    throw "PodiumBoard actuel introuvable. Aucun changement conserve."
}

# Importer TeamCrest pour afficher le blason de l'equipe choisie a cote du nom.
if ($content -match 'import \{ PlayerCard, type PlayerCardData, type ExtraBadge \} from "@/components/prono/PlayerCard";') {
    $content = $content.Replace(
      'import { PlayerCard, type PlayerCardData, type ExtraBadge } from "@/components/prono/PlayerCard";',
      'import { PlayerCard, TeamCrest, type PlayerCardData, type ExtraBadge } from "@/components/prono/PlayerCard";'
    )
} elseif ($content -notmatch '\bTeamCrest\b.*from "@/components/prono/PlayerCard"') {
    $content = $content.Replace(
      'import { PlayerCard,',
      'import { PlayerCard, TeamCrest,'
    )
}
                    ...
                    >
                      ...
                      <div className="relative flex min-h-[265px] flex-col items-center justify-end px-4 pb-5 pt-5 text-center sm:min-h-[300px] lg:min-h-[330px]">
                        ...
                        <div className="mt-3 flex max-w-full items-center justify-center gap-2">
                          ...
                          {p.favoriteTeam && (
                            <TeamCrest team={p.favoriteTeam} size="size-6" />
                          )}
                        </div>
...
# 2) Grande boite classement #4+ avec en-tetes. Le PlayerCard existant reste
# responsable du joueur, du niveau et du logo du club.
$oldList = @'
            {others.length > 0 && (
...
# Remplacement robuste du bloc #4+ : on se base sur le marqueur
# `others.length > 0` et non sur l'indentation exacte du fichier.
$listPattern = '(?s)\{others\.length\s*>\s*0\s*&&\s*\(\s*<section\b.*?</section>\s*\)\}'
$listMatch = [regex]::Match($content, $listPattern)

if (!$listMatch.Success) {
    Copy-Item $backup $rankingFile -Force
    throw "Bloc liste #4+ introuvable (recherche robuste). Aucun changement conserve."
}

Write-Host "OK : Bloc liste #4+ detecte par recherche robuste." -ForegroundColor Green
$content = $content.Substring(0, $listMatch.Index) + $newList.Trim() + $content.Substring($listMatch.Index + $listMatch.Length)

# 3) La barre d'infos devient plus fine pour laisser le podium respirer.
$content = $content.Replace(
'className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-x-8 gap-y-3 rounded-2xl border border-white/10 bg-[#0d1322]/70 px-5 py-4 backdrop-blur-md sm:justify-between sm:gap-x-4"',
'className="mx-auto flex max-w-5xl flex-wrap items-center justify-center gap-x-6 gap-y-2 rounded-2xl border border-white/10 bg-[#080e1b]/80 px-4 py-3 backdrop-blur-md sm:justify-between sm:gap-x-4"'
)
...
$tests = @(
  @("Podium premium", $check -match 'PODIUM PREMIUM'),
  @("Stade podium", $check -match '/stadium-gold-bg\.png'),
  @("Photo de profil podium", $check -match 'p\.avatarUrl'),
  @("Logo equipe podium", $check -match 'TeamCrest team=\{p\.favoriteTeam\}'),
  @("Niveau podium", $check -match 'p\.careerLevel'),
  @("Points podium", $check -match 'p\.points'),
  @("Grand bloc liste", $check -match 'max-w-6xl overflow-hidden rounded-\[2rem\]'),
  @("PlayerCard conserve", $check -match '<PlayerCard')
)
...
foreach ($t in $tests) {
  if ($t[1]) { Write-Host "OK : $($t[0])" -ForegroundColor Green }
    ...
    Copy-Item $backup $rankingFile -Force
    throw "Verification echouee : $($t[0]). Fichier restaure."
  }
}
...
Write-Host " CLASSEMENT PREMIUM PODIUM : BUILD OK !" -ForegroundColor Green
Write-Host "=============================================" -ForegroundColor Green
