import VestiaireFloatingButton from "@/components/VestiaireFloatingButton";
    ...
    <div className="min-h-screen bg-[#030712] text-slate-100 relative flex flex-col font-sans">
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5 md:px-8">
          ...
          {/* gap/padding resserrés entre lg (1024px, seuil d'activation de cette
              nav) et xl (1280px) : avec 7-8 liens (dont "Trophées"), la nav au
              padding "confortable" d'origine (px-4) dépassait la largeur de
              l'écran pile à 1024px — logo + nav + badge/Quitter arrivaient à
              ~1042px pour ~960px disponibles, provoquant un scroll horizontal
              sur TOUTES les pages (bug préexistant, découvert en testant le
              palier 1024px demandé pour cette refonte). Restauré à l'identique
              à partir de xl, où la place ne manque plus. */}
          <nav className="hidden lg:flex items-center gap-1 xl:gap-1.5 bg-[#0d1322]/90 border border-slate-800 p-1.5 rounded-2xl shadow-inner">
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`flex items-center gap-1.5 xl:gap-2 px-2.5 xl:px-4 py-2 rounded-xl font-display text-xs font-bold transition-all ${
                    isActive
      ...
      {/* bottom via style inline : décale la nav au-dessus de la barre de
          ...
          env() vaut 0px par défaut, donc identique à avant sur tout le reste.
      ...
      >
        {/* justify-around centre les items tant qu'ils tiennent ; l'ajout de "Trophées"
            ...
            laissait vraiment plus assez de place. */}
        <nav className="flex items-center justify-around overflow-x-auto bg-[#060b16]/90 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-1.5 shadow-[0_10px_30px_rgba(0,0,0,0.8)] [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = currentPath === item.to;
            return (
