 ...
 * (`/admin?tab=...`) plutÃ´t que par un simple state local, pour rester
...
// L'onglet "JournÃ©es" a Ã©tÃ© fusionnÃ© dans "Bonus" (tirage bonus + gestion
// des championnats europÃ©ens) : un ancien lien/bookmark `?tab=journees`
// doit continuer Ã  pointer quelque part plutÃ´t que de retomber sur l'onglet
// par dÃ©faut.
const LEGACY_TAB_REDIRECTS: Record<string, AdminTab> = { journees: "bonus" };

export const Route = createFileRoute("/admin-bonus-persistant")({
  component: AdminPage,
...
// ============================================================
// DonnÃ©es club (rÃ©utilisÃ©es de la page pronostics)
// ============================================================
...
/** Forme normalisÃ©e consommÃ©e par EntityBadge, qu'il s'agisse d'une
 * Ã©quipe Supabase (`teams`) ou d'un club officiel L1 codÃ© en dur. */
type BadgeEntity = { name: string; shortName?: string | null; logoUrl?: string | null } | null;

/** Badge gÃ©nÃ©rique logo + nom, avec repli sur les initiales si le logo
 * est absent ou casse au chargement. RÃ©utilisÃ© par TeamBadge et ClubBadge
 * ci-dessous, qui ne font plus qu'adapter leur source de donnÃ©es au
...
// ============================================================
// Petits composants UI partagÃ©s (style du reste de l'app)
// ============================================================
...
function GhostButton({
  ...
  danger?: boolean;
  title?: string;
  /** LibellÃ© accessible explicite â€” indispensable quand le bouton n'affiche
   * qu'une icÃ´ne (ex. GhostButton danger de suppression). Vient s'ajouter
...
/** Pile de notifications non bloquantes, en bas d'Ã©cran, qui reprend le
 * style d'ErrorBanner. Remplace les alert() natifs du navigateur : chaque
 * toast disparaÃ®t de lui-mÃªme aprÃ¨s quelques secondes ou peut Ãªtre fermÃ©
 * manuellement. */
...
const TABLE_MISSING_HINT =
  "Table introuvable dans Supabase. ExÃ©cutez le script SQL fourni (supabase_admin_schema.sql) puis synchronisez.";

/** Extrait un message lisible d'une erreur `catch` typÃ©e `unknown` (pas de
 * `any`) â€” utilisÃ© par les handlers ajoutÃ©s ci-dessous (score inline,
 * onglet Bonus, RÃ©glages) ; le reste du fichier utilise encore
 * `catch (e: any)`, une convention prÃ©existante non touchÃ©e ici. */
function errorMessage(e: unknown, fallback: string): string {
  return e instanceof Error && e.message ? e.message : fallback;
}

/** Enregistre une erreur de chargement sous `key` sans Ã©craser un message
 * dÃ©jÃ  prÃ©sent pour la mÃªme clÃ© : les messages distincts se combinent, les
 * doublons ne sont pas rÃ©pÃ©tÃ©s. Ã‰vite qu'une source fasse disparaÃ®tre
 * l'erreur d'une autre source rattachÃ©e au mÃªme onglet. */
...
// PAGE ADMIN
// ============================================================
// AdminTab est dÃ©fini plus haut, Ã  cÃ´tÃ© de la Route.
...
const TABS: { id: AdminTab; label: string; icon: typeof Users }[] = [
  ...
  { id: "reglages", label: "RÃ©glages", icon: SettingsIcon },
];

function AdminPage() {
  // L'onglet actif vit dans l'URL (?tab=...) plutÃ´t que dans un simple
  // useState, pour rester adressable (lien partagÃ©, retour arriÃ¨re, etc.).
  ...
  // Nettoie l'URL d'un ancien lien/bookmark `?tab=journees` (onglet fusionnÃ©
  // dans Bonus) vers `?tab=bonus`, une fois le routeur montÃ©. validateSearch
  // fait dÃ©jÃ  pointer `activeTab` sur "bonus" immÃ©diatement ; ceci ne fait
  // que corriger la barre d'adresse pour ne pas perpÃ©tuer l'ancien lien.
  useEffect(() => {
  ...
  // Notifications non bloquantes (remplace les alert() natifs) â€” voir ToastStack.
  ...
  async function loadAll() {
    ...
    // Les 8 sources sont indÃ©pendantes : on les charge en parallÃ¨le plutÃ´t
    // qu'en sÃ©rie pour diviser le temps de chargement par ~8, et on garde
    // Promise.allSettled pour qu'un Ã©chec isolÃ© (ex. table manquante)
    // n'empÃªche pas les autres sources de s'afficher.
    const [
      playersResult,
      paymentsResult,
      matchesResult,
    ...
    if (teamsResult.status === "fulfilled") {
      ...
      // ClÃ© distincte de "matchs" : les Ã©quipes ne sont qu'un rÃ©fÃ©rentiel
      // utilisÃ© par le formulaire de l'onglet Matchs, pas l'onglet lui-mÃªme.
      // Les confondre affichait par le passÃ© le badge d'erreur sur le
      // mauvais onglet.
        ...
        <div className="mx-auto max-w-6xl space-y-6 pb-32">
          ...
          <section className="relative overflow-hidden rounded-3xl border border-emerald-500/20 bg-[#060b16] p-6 shadow-[0_0_60px_rgba(0,0,0,0.6)] sm:p-8">
            ...
            <div className="relative z-10 flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  Joueurs, paiements, matchs, journÃ©es et rÃ©glages de la saison {settings?.season ?? "2026-2027"}.
                </p>
              </div>
              ...
              <GhostButton onClick={handleSync} className="!px-4 !py-2.5">
                ...
                {syncSuccess ? "SynchronisÃ©" : "Synchroniser"}
              </GhostButton>
            </div>
            ...
            <div className="relative z-10 mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              <StatPill label="Cagnotte" value={`${totalCollected}â‚¬`} tone="text-gold" />
              <StatPill label="JournÃ©es ouvertes" value={String(openMatchdaysCount)} tone="text-mint" />
            </div>
          </section>
...
// ============================================================
// ðŸ‘¥ ONGLET JOUEURS (Mis Ã  jour avec Ã‰quipe favorite et DÃ©rogation admin - Phases 2, 3, 4)
// ============================================================
function PlayersTab({
  players,
  teams,
  ...
  async function submitEdit() {
    ...
    try {
      await updatePlayer(editing.id, {
        ...
        favorite_team_override: true, // Phase 4 : DÃ©rogation admin active lors de la modification admin
      });
      setEditing(null);
      await onChanged();
    } catch (e: any) {
  ...
  // Mise Ã  jour optimiste : le rÃ´le change Ã  l'Ã©cran immÃ©diatement, sans
  // attendre Supabase. On ne resynchronise (refetch complet) qu'en cas
  // d'erreur, pour revenir Ã  un Ã©tat garanti cohÃ©rent.
  async function toggleAdmin(player: Player) {
    ...
    try {
      ...
      notify(e.message ?? "Erreur lors de la mise Ã  jour.");
      await onChanged();
    } finally {
      setBusyId(null);
    }
  ...
  // Idem : le joueur disparaÃ®t du tableau tout de suite. En cas d'Ã©chec de
  // la suppression cÃ´tÃ© Supabase, on resynchronise pour le faire rÃ©apparaÃ®tre.
    ...
    <Card className="p-5">
          ...
          <table className="w-full min-w-[700px] border-collapse">
            <thead>
              <tr className="border-b border-slate-800 text-left font-mono text-[10px] uppercase tracking-widest text-slate-500">
                ...
                <th className="pb-2 pr-3">Ã‰quipe favorite</th>
                <th className="pb-2 pr-3">Statut</th>
                <th className="pb-2 pr-3">RÃ´le</th>
                ...
                <tr key={player.id} className="border-b border-slate-800/60 last:border-0 hover:bg-slate-900/40 transition-colors">
                  ...
                  <td className="py-3 pr-3">
                    {favoriteTeam ? (
                      <span className="inline-flex items-center gap-1 text-emerald-400 text-xs font-mono">
                        <Check size={14} /> {player.favorite_team_override ? "ModifiÃ© (Admin)" : "OK"}
                      </span>
                    ) : (
                  ...
                  <td className="py-3 pr-0 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <GhostButton onClick={() => toggleAdmin(player)} title="Basculer le rÃ´le">
                        {busyId === player.id ? (
                        ...
                        {player.is_admin ? "RÃ©trograder" : "Promouvoir"}
                      </GhostButton>
                      <GhostButton
                        danger
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          description={`${confirmDelete.pseudo ?? "Ce joueur"} sera dÃ©finitivement supprimÃ©, ainsi que ses pronostics et paiements associÃ©s.`}
...
function PaymentsTab({
  ...
  // Mise Ã  jour optimiste : le badge de statut change immÃ©diatement, on ne
  // resynchronise depuis Supabase qu'en cas d'Ã©chec de la requÃªte.
  ...
  async function saveAmount(payment: Payment) {
    ...
    try {
      ...
      notify(e.message ?? "Erreur lors de la mise Ã  jour du montant.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function handleGenerateMissing() {
    ...
    try {
      ...
      notify(e.message ?? "Erreur lors de la gÃ©nÃ©ration des paiements.");
    } finally {
      setGenerating(false);
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatPill label="PayÃ©" value={`${paidCount} / ${players.length}`} />
        <StatPill label="CollectÃ©" value={`${totalCollected}â‚¬`} tone="text-gold" />
        <StatPill label="Attendu" value={`${totalExpected}â‚¬`} tone="text-sky-400" />
        <StatPill label="Restant" value={`${Math.max(totalExpected - totalCollected, 0)}â‚¬`} tone="text-red-400" />
      </div>
      ...
      <Card className="p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h2 className="flex items-center gap-2 font-display text-lg font-bold uppercase tracking-wide text-white">
            ...
            <span className="ml-1 rounded-full border border-slate-700 bg-slate-800/60 px-2.5 py-0.5 font-mono text-[11px] font-bold text-slate-300">
              {paidCount} payÃ©{paidCount > 1 ? "s" : ""} / {players.length} inscrit{players.length > 1 ? "s" : ""}
            </span>
          </h2>
          {missingCount > 0 && (
            <PrimaryButton onClick={() => setConfirmGenerate(true)} disabled={generating}>
              ...
              GÃ©nÃ©rer {missingCount} paiement{missingCount > 1 ? "s" : ""} manquant{missingCount > 1 ? "s" : ""}
            </PrimaryButton>
          )}
        </div>
        ...
        {!error && payments.length === 0 ? (
          <p className="py-10 text-center text-sm text-slate-500">
            Aucun paiement enregistrÃ©. GÃ©nÃ¨re les paiements pour commencer.
          </p>
        ) : !error && filteredPayments.length === 0 ? (
          <p className="py-10 text-center text-sm text-slate-500">Aucun joueur ne correspond Ã  Â« {search} Â».</p>
        ) : (
          <div className="overflow-x-auto">
                    ...
                    <tr key={payment.id} className="border-b border-slate-800/60 last:border-0">
                      ...
                      <td className="py-3 pr-0 text-right">
                        <GhostButton onClick={() => togglePaid(payment)}>
                          ...
                          {payment.paid ? "Marquer non payÃ©" : "Marquer payÃ©"}
                        </GhostButton>
                      </td>
                    </tr>
                  );
      ...
      {confirmGenerate && (
        <ConfirmDialog
          title="GÃ©nÃ©rer les paiements manquants ?"
          description={`${missingCount} paiement${missingCount > 1 ? "s" : ""} de ${entryFee}â‚¬ ${
            missingCount > 1 ? "seront crÃ©Ã©s" : "sera crÃ©Ã©"
...
/** Statut d'affichage dÃ©rivÃ© (il n'existe pas de colonne "status" en base). */
...
/** DÃ©termine la journÃ©e Ã  afficher par dÃ©faut Ã  l'ouverture de l'onglet :
 * la journÃ©e "en cours" (aujourd'hui tombe dans sa plage de coups d'envoi,
 * + 3h de tolÃ©rance pour couvrir un match encore en jeu), sinon la
 * prochaine journÃ©e Ã  venir, sinon la derniÃ¨re journÃ©e jouÃ©e, sinon la
 * premiÃ¨re journÃ©e connue. */
...
function MatchesTab({
  ...
  // SÃ©lecteur de journÃ©e : "all" affiche tout, sinon l'id de la journÃ©e
  // choisie. `null` = pas encore initialisÃ© (le premier effet ci-dessous
  // pose la journÃ©e courante/prochaine dÃ¨s que les donnÃ©es sont lÃ ), pour
  // ne pas Ã©craser un choix dÃ©jÃ  fait par l'admin lors d'un simple refetch.
  ...
  // Cet onglet ne montre que la Ligue 1 : les autres championnats europÃ©ens
  // (synchronisÃ©s depuis l'onglet Bonus) portent un match_type distinct de
  // "LIGUE1" et vivent dans leur propre liste lÃ -bas.
  ...
  // Sauvegarde manuelle du score, y compris sur un match "Ã  venir" : les
  // deux champs prÃ©sents et numÃ©riques font basculer `finished` Ã  true
  // automatiquement (seul signal de statut portÃ© par le modÃ¨le actuel,
  ...
  async function saveScore(match: Match) {
    ...
    if ((home !== null && Number.isNaN(home)) || (away !== null && Number.isNaN(away))) {
      notify("Le score doit Ãªtre un nombre.");
      return;
    }
    if (home !== null && home < 0) return notify("Le score domicile ne peut pas Ãªtre nÃ©gatif.");
    if (away !== null && away < 0) return notify("Le score extÃ©rieur ne peut pas Ãªtre nÃ©gatif.");
  ...
  async function submitForm() {
    if (!form.home_team_id || !form.away_team_id || !form.kickoff) {
      notify("Renseigne les deux Ã©quipes et la date/heure du match.");
      return;
    }
    setSaving(true);
    try {
  ...
  // Optimiste : le match quitte la liste immÃ©diatement : on ne resynchronise
  // depuis Supabase que si la suppression Ã©choue rÃ©ellement cÃ´tÃ© serveur.
  async function confirmAndDelete() {
    if (!confirmDelete) return;
    const target = confirmDelete;
    setBusyId(target.id);
  ...
  // Synchronise via l'API Vercel /api/ligue1/matchs, puis rafraÃ®chit les
  // matchs et journÃ©es. Aucune Edge Function Supabase n'est utilisÃ©e ici.
  async function handleSyncFromApi() {
    setSyncingApi(true);
    try {
      const summary = await syncLigue1Matches();
      await Promise.all([onChanged(), refreshMatchdays()]);
      // Une ancienne erreur de chargement ne doit pas rester affichÃ©e aprÃ¨s
      // une synchronisation rÃ©ussie.
      clearErrors(["matchs", "bonus", "equipes"]);

      const parts = [`${summary.created} crÃ©Ã©(s)`, `${summary.updated} mis Ã  jour`];
      if (summary.skipped > 0) parts.push(`${summary.skipped} ignorÃ©(s)`);
      if (summary.matchdaysCreated > 0) parts.push(`${summary.matchdaysCreated} journÃ©e(s) crÃ©Ã©e(s)`);
      notify(`Synchronisation terminÃ©e : ${parts.join(", ")}.`);

      if (summary.warnings.length > 0) {
        summary.warnings.forEach((w) => console.warn("[sync-ligue1-matches]", w));
        notify(
          `${summary.warnings.length} Ã©quipe(s)/match(s) ignorÃ©(s) faute de correspondance dans team_api_mapping (dÃ©tail dans la console).`,
        );
      }
      if (summary.errors.length > 0) {
        summary.errors.forEach((err) => console.error("[sync-ligue1-matches]", err));
        notify(`${summary.errors.length} erreur(s) pendant la synchronisation (dÃ©tail dans la console).`);
      }
    } catch (e: any) {
  ...
  return (
    <Card className="p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {syncingApi ? "Synchronisationâ€¦" : "Synchroniser depuis football-data.org"}
          </GhostButton>
      ...
      {/* Erreur distincte : les Ã©quipes ne se chargent pas forcÃ©ment en
          mÃªme temps que les matchs (bug corrigÃ© â€” voir addError cÃ´tÃ© page). */}
      {teamsError && (
        <div className="mb-4">
      ...
      {!error && visibleMatches.length === 0 ? (
        <p className="py-10 text-center text-sm text-slate-500">
          {ligue1Matches.length === 0 ? "Aucun match enregistrÃ© pour le moment." : "Aucun match pour cette journÃ©e."}
        </p>
      ) : (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs text-slate-400">
                  {match.kickoff ? new Date(match.kickoff).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" }) : "â€”"}
                </span>
                <StatusBadge status={matchDisplayStatus(match)} />

                {/* Score inline : Ã©ditable mÃªme sur un match "Ã  venir" â€” la
                    sauvegarde bascule automatiquement `finished` Ã  true dÃ¨s
                    que les deux scores sont renseignÃ©s (voir saveScore). */}
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    onBlur={() => hasDraftEdit && saveScore(match)}
                    onKeyDown={(e) => e.key === "Enter" && saveScore(match)}
                    aria-label={`Score extÃ©rieur ${teamOf(teams, match.away_team_id)?.name ?? "?"}`}
                    className="w-12 rounded-lg border border-slate-700 bg-[#060b16] px-1.5 py-1 text-center text-xs font-bold text-slate-100 outline-none focus:border-emerald-500/60"
                  />
                  {hasDraftEdit && (
                    <GhostButton
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-slate-700 bg-[#0d1322] px-3 py-2 text-sm text-slate-100 outline-none focus:border-emerald-500/60"
                >
                  <option value="scheduled">Ã€ venir / en cours</option>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; className: string }> = {
    scheduled: { label: "Ã€ venir", className: "border-sky-500/30 bg-sky-500/10 text-sky-400" },
    live: { label: "En direct", className: "border-red-500/30 bg-red-500/10 text-red-400 animate-pulse" },
    finished: { label: "TerminÃ©", className: "border-slate-700 bg-slate-800/60 text-slate-400" },
  };
...
// ðŸŽ ONGLET BONUS
// ============================================================
// Remplace l'ancien onglet "JournÃ©es" : point d'entrÃ©e unique pour tout ce
// qui touche au bonus (tirage de la sÃ©lection premium) et aux championnats
// hors Ligue 1 (activation, synchronisation football-data.org). Le
// verrouillage des journÃ©es (deadline/is_finished) a depuis Ã©tÃ© extrait
// dans son propre onglet, voir MatchdayLockTab.
/** Convertit un timestamp ISO en UTC (tel que renvoyÃ© par Supabase pour un
 * `timestamptz`, ex. "2026-08-21T04:00:00+00:00") vers le format attendu
 * par <input type="datetime-local"> â€” en heure locale Europe/Paris.
 *
 * BUG corrigÃ© ici (historique) : un `.slice(0, 16)` naÃ¯f sur la chaÃ®ne UTC
 * ("2026-08-21T04:00") rÃ©utilisait telles quelles les heures UTC comme si
 * elles Ã©taient dÃ©jÃ  en heure locale. `<input type="datetime-local">` n'a
 * aucune notion de fuseau â€” il affiche/interprÃ¨te toujours sa valeur comme
 * de l'heure locale.
 *
 * Le fix suivant utilisait `new Date(iso).getHours()`, donc l'heure LOCALE
 * DU NAVIGATEUR â€” correct uniquement si l'admin est physiquement/OS rÃ©glÃ©
 * sur Europe/Paris. RemplacÃ© par `utcIsoToParisLocalInput`
 * (bonusSelectionService.ts), qui calcule l'heure de Paris via Intl quel
 * que soit le fuseau de la machine qui exÃ©cute le code â€” cohÃ©rent avec
 * `isWithinBonusPeriod`, qui compare dÃ©sormais aussi en Europe/Paris
...
function BonusTab({
  ...
  // SÃ©lection bonus : Ã©tat local pour cette premiÃ¨re Ã©tape.
  // La persistance Supabase sera branchÃ©e dans adminService aprÃ¨s validation de l'UI.
  ...
  // Championnats (grandes ligues europÃ©ennes hors Ligue 1) : la liste
  // disponible vient de football-data.org, pas d'une constante codÃ©e en dur.
  ...
  // PÃ©riode d'Ã©ligibilitÃ© des matchs au tirage bonus (app_settings â€”
  // persistÃ©e pour survivre entre sessions, voir generateBonusForDay).
  // Sauvegarde au blur, comme les autres champs Ã©ditables inline de
  // l'admin (score de match, montant de paiement) â€” pas de bouton dÃ©diÃ©.
  ...
  // parisLocalToUtcIso (Intl, indÃ©pendant du fuseau de la machine) â€” jamais
  // via `new Date(periodStart).toISOString()`, qui utiliserait le fuseau du
  // navigateur exÃ©cutant le code.
  ...
  async function handleSavePeriod(requireBothDates = false): Promise<boolean> {
    if (requireBothDates && (!periodStart || !periodEnd)) {
      notify("Choisis une date de dÃ©but ET une date de fin avant de gÃ©nÃ©rer les matchs bonus.");
      return false;
    }

    if (periodStart && !periodStartIso) {
      notify("La date de dÃ©but bonus est invalide.");
      return false;
    }
    ...
    if (periodStartIso && periodEndIso && new Date(periodStartIso) >= new Date(periodEndIso)) {
      notify("La fin de la pÃ©riode doit Ãªtre aprÃ¨s le dÃ©but.");
      return false;
    }
    ...
    try {
      ...
      notify(errorMessage(e, "Erreur lors de l'enregistrement de la pÃ©riode."));
      return false;
    }
  }

  /** Un match est Ã©ligible au tirage s'il n'y a pas de pÃ©riode dÃ©finie, ou
   * si son coup d'envoi (UTC, football-data.org) tombe dans la fenÃªtre
   * [periodStartIso, periodEndIso] (bornes incluses, comparaison de vrais
   * timestamps via isMatchInWindow â€” jamais une comparaison de chaÃ®nes).
   * Pas de coup d'envoi connu = exclu dÃ¨s qu'une pÃ©riode est active (on ne
   * peut pas vÃ©rifier). */
  ...
  async function toggleCompetitionActive(discovered: DiscoveredCompetition) {
    ...
    try {
      ...
      notify(`${discovered.name} ${nextActive ? "activÃ©" : "dÃ©sactivÃ©"}.`);
    } catch (e) {
  ...
  async function handleSyncCompetition(discovered: DiscoveredCompetition) {
    ...
    try {
      ...
      if (summary.errors.length > 0) {
        ...
        notify(`${summary.errors.length} erreur(s) pendant la synchronisation de ${discovered.name} (dÃ©tail dans la console).`);
      }
    } catch (e) {
  ...
  /** Tous les matchs synchronisÃ©s d'un championnat, indÃ©pendamment de son
   * propre numÃ©ro de journÃ©e : les calendriers ne sont pas alignÃ©s entre
   * pays (le "matchday 1" anglais ne tombe pas la mÃªme semaine que le
   * "J1" franÃ§ais, voire le mÃªme mois selon le championnat). BUG corrigÃ©
   * ici â€” l'ancienne version filtrait par `candidateDay.number ===
   * md.number` (le numÃ©ro de la journÃ©e Ligue 1 sÃ©lectionnÃ©e), ce qui ne
   * pouvait retourner un rÃ©sultat que si les deux championnats avaient
   * par coÃ¯ncidence la mÃªme numÃ©rotation. Seule la pÃ©riode choisie
   * (isWithinBonusPeriod, ci-dessous) doit dÃ©limiter les matchs
   * Ã©ligibles au tirage, jamais un numÃ©ro de journÃ©e Ã©tranger. */
  ...
  function toBonusMatch(match: Match): Match {
    // BUG corrigÃ© ici â€” `teams` ne contient que les clubs de Ligue 1
    // (OFFICIAL_L1_CLUBS) : pour un match Ã©tranger (PL/PD/SA/BL1), la
    // recherche par home_team_id/away_team_id Ã©choue toujours et retombait
    // sur "", Ã©crasant le nom dÃ©jÃ  correct enregistrÃ© lors de la synchro
    // football-data.org (adminService.ts, syncCompetitionMatches). RÃ©sultat
    // : scoreBonusCandidate rejetait chaque match Ã©tranger (home_team/
    // away_team vides), donc aucun candidat n'Ã©tait jamais retenu. On garde
    // dÃ©sormais la valeur dÃ©jÃ  prÃ©sente sur le match en repli.
  ...
  async function generateBonusForDay(md: Matchday) {
    // Les dates choisies par l'admin sont la source de vÃ©ritÃ© du tirage.
    // On les enregistre AVANT de lancer la sÃ©lection pour Ã©viter qu'un clic
    // sur GÃ©nÃ©rer avec des dates fraÃ®chement modifiÃ©es utilise encore les
    ...
    try {
      ...
      // repartir en chasse en base Ã  chaque fois : absence totale de
      // synchronisation vs matchs existants mais hors de la pÃ©riode choisie
      ...
      (["PL", "PD", "SA", "BL1"] as BonusCompetitionCode[]).forEach((code) => {
        ...
        if (allForCompetition.length === 0) {
          misses.push(`${label} : aucun match synchronisÃ© (onglet Bonus â†’ Championnats)`);
          return;
        }
        ...
        const nextKickoffLabel = nextKickoff
          ...
          : null;
        misses.push(
          `${label} : ${allForCompetition.length} match(s) synchronisÃ©(s), aucun dans la pÃ©riode` +
      ...
      if (Object.keys(next).length === 0) {
        ...
        notify(
          `Aucun match Ã©ligible pour J${md.number} â€” fenÃªtre demandÃ©e${windowLabel ? ` ${windowLabel}` : ""} â€” ${misses.join(" Â· ")}`,
        );
        return;
      }

      // La base est la source de vÃ©ritÃ© : on sauvegarde d'abord, puis on
      // met Ã  jour l'Ã©tat local uniquement si Supabase confirme l'Ã©criture.
      await saveBonusSelections(md.id, Object.values(next));
      setBonusSelections((prev) => ({ ...prev, [key]: next }));
      setReplacingBonus(null);
      if (misses.length > 0) {
        notify(`${Object.keys(next).length}/4 sÃ©lectionnÃ©s pour J${md.number}. Manquants â€” ${misses.join(" Â· ")}`);
      } else {
        notify(`${Object.keys(next).length}/4 Matchs bonus sÃ©lectionnÃ©s pour J${md.number}.`);
      }
    } finally {
  ...
  async function replaceBonusForDay(md: Matchday, code: BonusCompetitionCode, match: Match) {
    ...
    try {
      ...
      notify(errorMessage(e, "Erreur lors de l'enregistrement du remplacement bonus."));
    }
  }
  ...
  async function removeBonusDraw(md: Matchday) {
    ...
    try {
      ...
      setReplacingBonus(null);
      notify(`Tirage bonus de J${md.number} retirÃ©.`);
    } catch (e) {
  ...
  // Saisie du rÃ©sultat (score rÃ©el du match, "2-1"...) pour le match bonus
  // retenu de chaque championnat. Ne PAS confondre avec `candidate.score`
  // (le score de PERTINENCE 0-100 de l'algorithme de sÃ©lection, affichÃ©
  // juste au-dessus) â€” c'Ã©tait l'ambiguÃ¯tÃ© de fond du bug signalÃ© : l'onglet
  // Matchs n'affiche que la Ligue 1 (`match_type === "LIGUE1"`), donc les
  // matchs PL/PD/SA/BL1 sÃ©lectionnÃ©s ici n'avaient tout simplement AUCUNE UI
  // pour saisir leur rÃ©sultat nulle part dans l'admin. RÃ©utilise updateMatch
  // et le mÃªme schÃ©ma que MatchesTab.saveScore (finished bascule Ã  true
  // seulement quand les deux scores sont renseignÃ©s).
  const [bonusScoreDrafts, setBonusScoreDrafts] = useState<Record<string, { home: string; away: string }>>({});
  ...
  function setBonusScoreDraft(match: Match, patch: Partial<{ home: string; away: string }>) {
    setBonusScoreDrafts((prev) => ({ ...prev, [match.id]: { ...bonusScoreDraftFor(match), ...patch } }));
  }

  async function saveBonusScore(code: BonusCompetitionCode, match: Match) {
    const draft = bonusScoreDraftFor(match);
    const homeRaw = draft.home.trim();
    const awayRaw = draft.away.trim();
    const home = homeRaw === "" ? null : Number(homeRaw);
    const away = awayRaw === "" ? null : Number(awayRaw);
    ...
    try {
      ...
      // ReflÃ¨te immÃ©diatement le nouveau score dans l'Ã©tat local de
      // sÃ©lection (bonusSelections) â€” sinon la carte resterait affichÃ©e
      // avec l'ancien score jusqu'Ã  une rÃ©gÃ©nÃ©ration complÃ¨te du tirage,
      // puisque ce state n'est rÃ©-hydratÃ© depuis Supabase qu'une fois par
      // journÃ©e (voir l'effet plus haut).
      if (selectedBonusKey) {
        setBonusSelections((prev) => {
      ...
      await onChanged();
      notify("Score du match bonus enregistrÃ©.");
    } catch (e) {
      notify(errorMessage(e, "Erreur lors de l'enregistrement du score bonus."));
    } finally {
  ...
  // SÃ©lection bonus : une seule journÃ©e Ligue 1 Ã  la fois (dropdown),
  // plutÃ´t qu'une carte par journÃ©e â€” sert uniquement Ã  nommer/stocker le
  // tirage (clÃ© season_id:number) ; les matchs Ã©ligibles de chaque
  // championnat sont eux dÃ©terminÃ©s par la pÃ©riode (bonusMatchesForCompetition
  // + isWithinBonusPeriod), pas par le numÃ©ro de journÃ©e de la Ligue 1.
      ...
      try {
        const entries = await Promise.all(
          ligue1Matchdays.map(async (md) => {
            ...
            return [`${md.season_id}:${md.number}`, hydrated] as const;
          }),
        );

        if (!cancelled) {
          setBonusSelections(Object.fromEntries(entries));
        }
      } catch (e) {
  ...
  // Validation explicite de la journÃ©e avant tirage : le dropdown reste
  // librement modifiable, mais "GÃ©nÃ©rer" ne doit consommer que la journÃ©e
  // que l'admin a confirmÃ©e en cliquant "Valider" â€” jamais la valeur brute
  // du dropdown au moment du clic. isBonusMatchdayValidated est dÃ©rivÃ© (pas
  // un state sÃ©parÃ©) : il retombe Ã  false automatiquement dÃ¨s que le
  // dropdown change, sans effet supplÃ©mentaire.
    ...
    <div className="space-y-4">
      <Card className="p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Championnats disponibles sur ton compte football-data.org â€” active ceux Ã  synchroniser pour la sÃ©lection bonus.
            </p>
          </div>
        ...
        {competitionsLoaded && availableCompetitions.length === 0 && (
          <p className="py-6 text-center text-sm text-slate-500">Aucun championnat renvoyÃ© par football-data.org.</p>
        )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="font-mono text-[10px] text-slate-500">
                        {discovered.code}
                        {discovered.country ? ` Â· ${discovered.country}` : ""}
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-slate-700 bg-slate-800/60 px-2.5 py-1 font-mono text-[10px] font-bold text-slate-300">
                      {count} match{count > 1 ? "s" : ""} synchronisÃ©{count > 1 ? "s" : ""}
                    </span>
                    <GhostButton
                      onClick={() => toggleCompetitionActive(discovered)}
                      disabled={togglingCode === discovered.code}
                      title={isActive ? "DÃ©sactiver ce championnat" : "Activer ce championnat"}
                    >
      ...
      <Card className="p-5 border-sky-500/20">
        <div className="mb-1 font-mono text-[11px] font-bold uppercase tracking-widest text-sky-400">
          SÃ©lection bonus premium
        </div>
        <h2 className="font-display text-xl font-black text-white">4 meilleures affiches, une par championnat</h2>
        <p className="mt-1 text-xs text-slate-500">
          Le site analyse automatiquement le prestige des Ã©quipes, l'Ã©quilibre du match, les rivalitÃ©s et l'horaire.
        </p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-[1fr_1fr_1fr_auto_auto] lg:items-end">
          <div>
            ...
            >
              {ligue1Matchdays.length === 0 && <option value="">Aucune journÃ©e synchronisÃ©e</option>}
            ...
            <div className="mt-2">
              {isBonusMatchdayValidated ? (
                <span className="inline-flex items-center gap-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 font-mono text-[10px] font-bold text-emerald-300">
                  <CheckCircle2 size={12} />
                  JournÃ©e {selectedBonusMatchday?.number} validÃ©e âœ“
                </span>
              ) : (
                <GhostButton
                ...
                >
                  <CheckCircle2 size={12} />
                  Valider journÃ©e {selectedBonusMatchday?.number ?? ""}
                </GhostButton>
              )}
            </div>
          </div>
          ...
          <button
            ...
            title={!isBonusMatchdayValidated ? "Valide la journÃ©e choisie avant de gÃ©nÃ©rer." : undefined}
          ...
          >
            ...
            GÃ©nÃ©rer la sÃ©lection premium
          </button>
          <GhostButton
            danger
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
          <p className="text-[10px] text-slate-500">
            Le tirage prend en compte TOUS les matchs synchronisÃ©s dont le coup d'envoi est compris entre ces deux dates,
            inclusivement. Les journÃ©es des championnats Ã©trangers ne sont jamais utilisÃ©es pour filtrer.
          </p>
          <span className="rounded-full border border-sky-500/20 bg-sky-500/10 px-2.5 py-1 font-mono text-[10px] font-bold text-sky-300">
            {periodStart && periodEnd ? "PÃ©riode active" : "Dates obligatoires"}
          </span>
        </div>
                ...
                {candidate ? (
                  <>
                    <div className="text-sm font-bold text-white">
                      {candidate.match.home_team} <span className="text-slate-600">â€”</span> {candidate.match.away_team}
                    </div>
                    <div className="mt-1 text-[10px] text-slate-500">
                      {candidate.match.kickoff
                        ? new Date(candidate.match.kickoff).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })
                        : "Date inconnue"}
                      {candidate.reasons.length ? ` Â· ${candidate.reasons.join(" Â· ")}` : " Â· Affiche retenue automatiquement"}
                    </div>

                    {/* Score rÃ©el du match bonus â€” absent jusqu'ici de tout
                        l'admin (l'onglet Matchs ne liste que la Ligue 1). */}
                    <div className="mt-3 border-t border-slate-800 pt-3">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          min={0}
                          inputMode="numeric"
                          value={bonusScoreDraftFor(candidate.match).home}
                          onChange={(e) => setBonusScoreDraft(candidate.match, { home: e.target.value })}
                          placeholder="-"
                          aria-label={`Score ${candidate.match.home_team}`}
                          className="w-14 rounded-lg border border-slate-700 bg-[#050913] px-2 py-1.5 text-center text-sm font-bold text-white outline-none focus:border-sky-500/60"
                        />
                        ...
                        <input
                          ...
                          min={0}
                          inputMode="numeric"
                          value={bonusScoreDraftFor(candidate.match).away}
                          onChange={(e) => setBonusScoreDraft(candidate.match, { away: e.target.value })}
                          placeholder="-"
                          aria-label={`Score ${candidate.match.away_team}`}
                          className="w-14 rounded-lg border border-slate-700 bg-[#050913] px-2 py-1.5 text-center text-sm font-bold text-white outline-none focus:border-sky-500/60"
                        />
                        <button
                          type="button"
                          onClick={() => void saveBonusScore(code, candidate.match)}
                          disabled={savingBonusScoreId === candidate.match.id}
                          className="ml-auto inline-flex items-center gap-1 rounded-lg bg-sky-500 px-2.5 py-1.5 font-mono text-[10px] font-bold uppercase text-slate-950 transition-all hover:bg-sky-400 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          {savingBonusScoreId === candidate.match.id ? (
                      ...
                      {candidate.match.finished && candidate.match.home_score != null && candidate.match.away_score != null && (
                        <div className="mt-1.5 font-mono text-[10px] text-emerald-400">
                          RÃ©sultat enregistrÃ© : {candidate.match.home_score} â€“ {candidate.match.away_score}
                        </div>
                      )}
                    </div>
                  </>
                        ...
                        {alternatives.map((match) => {
                          ...
                          return (
                            <button
                              key={match.id}
                              type="button"
                              onClick={() => selectedBonusMatchday && void replaceBonusForDay(selectedBonusMatchday, code, match)}
                            ...
                            >
                              <span className="text-xs font-semibold text-slate-200">
                                {scored.match.home_team} â€” {scored.match.away_team}
                              </span>
...
// ============================================================
// ðŸ” ONGLET VERROUILLAGE
// ============================================================
// Extrait de BonusTab : seul endroit qui Ã©crit matchdays.deadline /
// deadline_mode / is_finished, lu par src/routes/pronostics.tsx pour
// bloquer les pronostics cÃ´tÃ© joueur â€” logique inchangÃ©e, uniquement
// dÃ©placÃ©e dans son propre onglet.
function MatchdayLockTab({
  ...
  async function submitEdit() {
    ...
    if (Number.isNaN(parsedNumber)) {
      notify("Le numÃ©ro de la journÃ©e doit Ãªtre un nombre.");
      return;
    }
    setSaving(true);
    try {
      ...
      notify(e.message ?? "Erreur lors de la modification de la journÃ©e.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleManualLock(md: Matchday) {
    ...
    try {
      ...
      await onChanged();
      notify(`ðŸ” J${md.number} verrouillÃ©e en mode manuel.`);
    } catch (e: any) {
  ...
  async function handleAutoMinusOne(md: Matchday) {
    ...
    try {
      ...
      await onChanged();
      notify(`âš¡ J${md.number} : verrouillage automatique -1 min activÃ©.`);
    } catch (e: any) {
  ...
  async function handleClearLock(md: Matchday) {
    ...
    try {
      ...
      await onChanged();
      notify(`ðŸ”“ Verrouillage retirÃ© pour J${md.number}.`);
    } catch (e: any) {
  ...
  // le badge EN COURS/TERMINÃ‰E change tout de suite.
  ...
  // Optimiste : la journÃ©e disparaÃ®t de la liste immÃ©diatement, on ne
    ...
    <div className="space-y-4">
      <Card className="p-5">
        ...
        {!error && matchdays.length === 0 ? (
          <p className="py-10 text-center text-sm text-slate-500">Aucune journÃ©e crÃ©Ã©e pour le moment.</p>
        ) : (
          <div className="space-y-2">
            {matchdays.map((md) => (
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-slate-100">
                      {(md.competition_id ? competitionsById.get(md.competition_id) : undefined)?.name ?? "CompÃ©tition inconnue"}
                    </div>
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 font-mono text-[10px] text-slate-500">
                      <span>
                        {matchCountByMatchdayId.get(md.id) ?? 0} match(s) Â· saison{" "}
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <GhostButton
                    ...
                    title="DÃ©finir une date/heure limite"
                    ariaLabel={`DÃ©finir une limite manuelle pour J${md.number}`}
                  >
                    {lockBusyId === md.id ? "â€¦" : "ðŸ”"}
                    Manuel
                  </GhostButton>
                  ...
                  <GhostButton onClick={() => toggleFinished(md)}>
                    ...
                    {md.is_finished ? "Rouvrir" : "ClÃ´turer"}
                  </GhostButton>
                  <GhostButton onClick={() => openEdit(md)} title="Modifier" ariaLabel={`Modifier la journÃ©e J${md.number}`}>
                    <Pencil size={12} />
                  </GhostButton>
                  <GhostButton
                    danger
                    ...
                    title="Supprimer"
                    ariaLabel={`Supprimer la journÃ©e J${md.number}`}
                  >
                    <Trash2 size={12} />
                  </GhostButton>
                </div>
      ...
      {confirmDelete && (
        <ConfirmDialog
          ...
          description={`La journÃ©e J${confirmDelete.number} sera supprimÃ©e. Les matchs dÃ©jÃ  rattachÃ©s perdront leur journÃ©e.`}
          confirmLabel="Supprimer"
          busy={busyId === confirmDelete.id}
          onCancel={() => setConfirmDelete(null)}
          onConfirm={confirmAndDelete}
...
/** Liste compacte, triÃ©e par journÃ©e, des matchs synchronisÃ©s pour un
 * championnat de l'onglet Bonus â€” Ã©quivalent allÃ©gÃ© du tableau de l'onglet
 * Matchs (mÃªmes scores Ã©ditables inline), sans crÃ©ation/suppression : ces
 * matchs n'existent que via la synchronisation football-data.org. Pas de
 * badge logo (aucune Ã©quipe hors Ligue 1 n'est en base, voir
 * syncCompetitionMatches) : le nom brut renvoyÃ© par l'API suffit ici. */
function CompetitionMatchesList({
  ...
  // SÃ©lecteur de journÃ©e scopÃ© Ã  ce championnat (pas de J1-J34 fixe : le
  // numÃ©ro vient tel quel du champ `matchday` renvoyÃ© par football-data.org
  // pour CE championnat, voir syncCompetitionMatches). Si plusieurs
  // championnats sont dÃ©veloppÃ©s en mÃªme temps dans l'onglet Bonus, chaque
  // instance de CompetitionMatchesList a son propre state â€” le filtre ne
  // s'applique donc qu'au championnat affichÃ© ici, jamais globalement.
  ...
  if (sorted.length === 0) {
    return <p className="py-4 text-center text-xs text-slate-500">Aucun match synchronisÃ© pour ce championnat.</p>;
  }
      ...
      {sortedMatchdays.length > 1 && (
        <div className="mb-2 flex items-center gap-1">
          <GhostButton
            ...
            title="JournÃ©e prÃ©cÃ©dente"
            ariaLabel="JournÃ©e prÃ©cÃ©dente"
          ...
          <GhostButton
            ...
            title="JournÃ©e suivante"
            ariaLabel="JournÃ©e suivante"
            className="!px-1.5 !py-1"
          >
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono text-[10px] text-slate-500">
                {match.kickoff ? new Date(match.kickoff).toLocaleDateString("fr-FR", { dateStyle: "short" }) : "â€”"}
              </span>
              <StatusBadge status={matchDisplayStatus(match)} />
              <input
                type="number"
              ...
              <input
                ...
                onBlur={() => hasDraftEdit && saveScore(match)}
                onKeyDown={(e) => e.key === "Enter" && saveScore(match)}
                aria-label={`Score extÃ©rieur ${match.away_team ?? "?"}`}
                className="w-10 rounded border border-slate-700 bg-[#0d1322] px-1 py-0.5 text-center text-xs font-bold text-slate-100 outline-none focus:border-emerald-500/60"
              />
              {hasDraftEdit && (
...
// ============================================================
/** Champ numÃ©rique gÃ©nÃ©rique : on garde la saisie en texte tant que le
 * champ est Ã©ditÃ© (comme le reste de l'admin â€” voir montant de paiement,
 * score de match) pour ne pas gÃªner la frappe d'un nombre dÃ©cimal ou d'un
 * champ vidÃ© temporairement ; la conversion en nombre a lieu Ã 
 * l'enregistrement. */
...
function SettingsTab({
  ...
  async function handleSave() {
    ...
    try {
      ...
      await onChanged();
      setSaved(true);
      notify("RÃ©glages enregistrÃ©s.");
      setTimeout(() => setSaved(false), 2500);
    } catch (e) {
      notify(errorMessage(e, "Erreur lors de l'enregistrement des rÃ©glages."));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleTestApiConnection() {
    ...
    try {
      const list = await getAvailableCompetitions();
      notify(`âœ… Connexion football-data.org OK â€” ${list.length} championnat(s) disponible(s) sur ce compte.`);
    } catch (e) {
      notify(errorMessage(e, "âŒ Connexion Ã  football-data.org impossible."));
    } finally {
      setTestingApi(false);
    }
  }
    ...
    <div className="space-y-4">
      ...
      <Card className="p-5">
        ...
        <p className="mb-4 text-xs text-slate-500">
          Ces valeurs sont sauvegardÃ©es, mais aucun moteur de calcul de points n'existe encore dans le code ni en
          base Ã  ce jour (vÃ©rifiÃ©) â€” elles ne s'appliquent pas encore automatiquement aux pronostics.
        </p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <NumberField label="Score exact (points)" value={scoreExact} onChange={setScoreExact} icon={CheckCircle2} />
          <NumberField
            label="Bon rÃ©sultat 1N2 (points)"
            value={correctResult}
            onChange={setCorrectResult}
            icon={Check}
            hint="Bon sens du rÃ©sultat, sans le score exact."
          />
          <NumberField
            ...
            icon={Plus}
            hint="Nombre de buts d'une des deux Ã©quipes devinÃ© juste."
          />
        </div>
      </Card>
      ...
      <Card className="p-5 border-amber-500/30 bg-[#0d1322]">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <NumberField
            ...
            icon={Gift}
            hint="0 = aucun bonus. Non appliquÃ© automatiquement aujourd'hui (voir barÃ¨me de points)."
          />
          <div className="flex flex-col justify-end">
            <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-800 bg-[#060b16] p-2.5 transition-colors hover:border-slate-700">
              ...
              <span className="flex items-center gap-1.5 text-sm font-medium text-white">
                <Lock size={14} className="text-amber-400" /> Verrouillage automatique Ã  la date limite
              </span>
            </label>
          </div>
        </div>
      ...
      <Card className="p-5">
        ...
        <p className="mb-4 text-xs text-slate-500">
          Les championnats couverts se pilotent depuis l'onglet Bonus (activer/dÃ©sactiver) â€” ici, la cadence des
          tirages et un barÃ¨me spÃ©cifique optionnel pour les matchs bonus.
        </p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <NumberField
            label="Tirages bonus par pÃ©riode"
            value={bonusDrawsPerPeriod}
            onChange={setBonusDrawsPerPeriod}
            icon={RefreshCw}
          />
          <NumberField
            label="Points pronostic bonus (si diffÃ©rent du barÃ¨me standard)"
            value={bonusMatchPoints}
            onChange={setBonusMatchPoints}
            icon={Gift}
            hint="Laisser vide pour appliquer le barÃ¨me standard ci-dessus."
          />
        </div>
      </Card>
      ...
      <Card className="p-5">
        ...
        <p className="mb-4 text-xs text-slate-500">
          Valeur par dÃ©faut utilisÃ©e par le verrouillage automatique Â« Auto âˆ’1 min Â» des journÃ©es (onglet Bonus).
        </p>
      ...
      <Card className={`p-5 ${maintenanceMode ? "border-red-500/40 bg-red-500/[0.03]" : ""}`}>
        ...
        <p className="mb-3 text-xs text-slate-500">
          RÃ©glage sauvegardÃ© â€” Ã  brancher cÃ´tÃ© page Pronostics si tu veux qu'il bloque rÃ©ellement les saisies (pas
          encore lu par le code aujourd'hui).
        </p>
        <TextInput
          value={maintenanceMessage}
          onChange={(e) => setMaintenanceMessage(e.target.value)}
          placeholder="Message affichÃ© aux joueurs pendant la maintenance (optionnel)"
        />
      </Card>

      {/* ================= SAISON & PARAMÃˆTRES GÃ‰NÃ‰RAUX ================= */}
      <Card className="p-5">
        <h2 className="mb-4 flex items-center gap-2 font-display text-lg font-bold uppercase tracking-wide text-white">
          <SettingsIcon size={18} className="text-emerald-400" />
          Saison &amp; paramÃ¨tres gÃ©nÃ©raux
        </h2>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <NumberField label="Droit d'entrÃ©e (â‚¬ / joueur)" value={entryFee} onChange={setEntryFee} />
          <div>
            <label className="mb-1 block font-mono text-[10px] uppercase tracking-widest text-slate-500">
            ...
            <p className="mt-1 text-[10px] text-slate-500">
              SauvegardÃ©, non encore branchÃ© dans le formatage des heures ci-dessous (toujours navigateur local).
            </p>
          </div>
          <div>
            <label className="mb-1 block font-mono text-[10px] uppercase tracking-widest text-slate-500">
      ...
      <Card className="p-5">
        <h2 className="mb-1 flex items-center gap-2 font-display text-lg font-bold uppercase tracking-wide text-white">
          <Globe size={18} className="text-emerald-400" />
          IntÃ©gration football-data.org
        </h2>
        <p className="mb-4 text-xs text-slate-500">
          ClÃ© API stockÃ©e cÃ´tÃ© serveur (variable d'environnement <code className="text-slate-400">FOOTBALL_DATA_API_TOKEN</code>),
          jamais exposÃ©e ici. Ce bouton vÃ©rifie juste que la connexion fonctionne.
        </p>
      ...
      <Card className="p-5">
        ...
        <p className="mb-4 text-sm text-slate-400">
          Recharge l'ensemble des donnÃ©es admin (joueurs, paiements, matchs, bonus, rÃ©glages) depuis Supabase.
        </p>
...
// ============================================================
// Composants gÃ©nÃ©riques : Modal & confirmation
// ============================================================
function Modal({
  title,
  children,
...
function ConfirmDialog({
  ...
  onCancel: () => void;
  onConfirm: () => void;
  /** "danger" (dÃ©faut, inchangÃ©) pour les suppressions ; "primary" pour une
   * confirmation d'action non destructive (ex. gÃ©nÃ©ration en masse), qui
   * reprend alors le vert Ã©meraude du reste de l'app au lieu du rouge. */
  tone?: "danger" | "primary";
}) {
